...
### 8. Split Button Component ⭐ NEW COMPONENT
...
- **Status**: Modified
- **Reusable Code**:
  - ✅ Complete new UI component (entire file): Can be directly used
  - ✅ Split button with left/right sections (lines 15-30): Compound button pattern
  - ✅ Conditional interactivity (lines 43-45): Clickable vs text-selectable states
  - ✅ Vertical separator component (lines 76-88): Visual divider

### 9. Frame Cache Hook ⭐ PERFORMANCE CRITICAL
...
- **Status**: Modified
- **Reusable Code**:
  - ✅ Scene-aware caching (line 41): sceneId parameter in getTimelineHash
  - ✅ Global singleton cache pattern (lines 23-27): HMR-safe shared cache
  - ✅ Timeline hash generation (lines 35-100): Efficient change detection
  - ✅ Hidden element filtering (lines 69-70): Skips hidden elements from cache
  - ✅ Frame cache data structure (lines 11-15): ImageData with hash and timestamp
  - ✅ Configurable cache parameters (lines 17-20): maxCacheSize, cacheResolution

### 10. Storage Service ⭐ CRITICAL FOR SCENES
...
- **Status**: Modified
- **Reusable Code**:
  - ✅ Scene-aware timeline storage (lines 56-72): Separate storage per scene
  - ✅ Scene serialization (lines 77-83): Convert Scene objects to/from storage
  - ✅ Project with scenes structure (lines 85-100): Updated project format
  - ✅ Dynamic DB naming for scenes (lines 63-65): `timeline-projectId-sceneId` pattern
  - ✅ Scene-specific timeline adapter (line 58): Optional sceneId parameter
  - ✅ currentSceneId tracking (line 92): Active scene management

### 11. Project Store ⭐ CRITICAL UPDATE
...
- **Status**: Modified
- **Reusable Code**:
  - ✅ Scene integration in project (lines 1, 7, 33-34): Imports and uses scene store
  - ✅ createMainScene helper (lines 14-22): Factory function for main scene
  - ✅ Default project with scenes (lines 24-43): Creates project with main scene
  - ✅ Scene array in project structure (line 33): scenes: [mainScene]
  - ✅ currentSceneId tracking (line 34): Active scene reference
  - ✅ Scene-aware project creation: All new projects include scenes

### 12. Scene Store ⭐ CORE SCENE MANAGEMENT
...
- **Status**: Modified
- **Reusable Code**:
  - ✅ Complete scene store implementation (entire file): Core scene management logic
  - ✅ Scene CRUD operations (lines 70-194): Create, delete, rename scenes
  - ✅ Scene switching with timeline save/load (lines 196-233): Seamless scene transitions
  - ✅ Main scene protection (lines 115-117): Prevents deletion of main scene
  - ✅ ensureMainScene helper (lines 12-25): Guarantees main scene exists
  - ✅ Scene-timeline integration (lines 210-220): Auto-saves timeline on switch
  - ✅ Project-scene synchronization (lines 83-98): Updates project with scene changes
  - ✅ Scene initialization (lines 267-310): Sets up scenes on project load

### 13. Project Types ⭐ TYPE DEFINITIONS
- **Original Path**: `apps/web/src/types/project.ts`
- **Our Repo Path**: `qcut/src/types/project.ts`
- **Downloaded File**: `project.ts`
- **Status**: Modified
- **Reusable Code**:
  - ✅ Scene interface (lines 5-11): Core scene data structure
  - ✅ TProject with scenes (lines 13-35): Updated project type with scene support
  - ✅ scenes array property (line 19): Array of Scene objects
  - ✅ currentSceneId property (line 20): Active scene tracking
  - ✅ isMain flag in Scene (line 8): Identifies primary scene
