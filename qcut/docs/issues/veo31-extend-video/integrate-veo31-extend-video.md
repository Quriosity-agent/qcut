...
- **Subtask 2 — AI constants:** ✅ COMPLETED. Added `veo31_fast_extend_video` and `veo31_extend_video` model entries to `apps/web/src/components/editor/media-panel/views/ai/constants/ai-constants.ts` in avatar section (~line 967-1011).
...
- **Subtask 6 — Avatar tab UI:** ✅ COMPLETED. Added extend-video props, model detection, and UI options section to `apps/web/src/components/editor/media-panel/views/ai/tabs/ai-avatar-tab.tsx` (~line 113-117 for props, ~line 198-200 for detection, ~line 602-666 for UI). Wired up to parent component in `apps/web/src/components/editor/media-panel/views/ai/index.tsx` (~line 325-326, 860-868). Added props to `UseAIGenerationProps` interface in `types/ai-types.ts` (~line 266-270) and hook destructuring in `use-ai-generation.ts` (~line 169-171, 1169-1171).
- **Subtask 7 — Cost calculator:** ✅ COMPLETED. Added `calculateVeo31ExtendCost()` function to `apps/web/src/components/editor/media-panel/views/ai/utils/ai-cost-calculators.ts` (~line 252-276).
- **Subtask 8 — Validation constants:** ✅ COMPLETED. Added `UPLOAD_CONSTANTS.MAX_EXTEND_VIDEO_DURATION_SECONDS`, `EXTEND_VIDEO_SUPPORTED_FORMATS`, `EXTEND_VIDEO_SUPPORTED_RESOLUTIONS`, `EXTEND_VIDEO_SUPPORTED_ASPECT_RATIOS`, `EXTEND_VIDEO_OUTPUT_DURATION_SECONDS` (~line 1159-1164) and `ERROR_MESSAGES.EXTEND_VIDEO_*` entries (~line 1304-1314) to `ai-constants.ts`.
- **Subtask 9 — Tests:** ✅ COMPLETED. Created `apps/web/src/components/editor/media-panel/views/ai/__tests__/extend-video.test.ts` with 20 passing tests covering model definitions, cost calculator, validation constants, and error messages.
