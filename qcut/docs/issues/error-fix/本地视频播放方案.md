...
1. **封装播放源选择函数**（已完成）
   - 位置：`qcut/apps/web/src/lib/media-source.ts`
   - 逻辑：
...
2. **播放器接入 blob 并管理清理**（已完成）
   - 适用文件：`qcut/apps/web/src/components/ui/video-player.tsx`
   - 改动：新增 `previousSrcRef` 和 `useEffect([src])`，在 `src` 变化或卸载时对 `blob:` URL 调用 `revokeObjectURL`，避免泄漏。
   - 做法：
...
3. **预览层使用本地优先源**（已完成）
   - 适用文件：`qcut/apps/web/src/components/editor/preview-panel.tsx`
   - 改动：两处 `VideoPlayer` 入口（约第 590 行、740 行）改为调用 `getVideoSource(mediaItem)`，优先 blob、缺失时回退远程；无源时显示“无可用视频源”占位。
