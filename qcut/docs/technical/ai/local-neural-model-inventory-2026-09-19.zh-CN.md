...
本页首先记录只读审计时的初始分类；同日后续研究见下方更新。未修改产品执行路径或调用收费云端接口。
...
**最新进展**见[第四阶段记录](../../../research/local-model-pytorch/PHASE4-20260919.zh-CN.md)：累计 18 个源资产中 21 个网络版本通过原生/已有原生记录对照，16 个 PT 包；19 份 ONNX 在 macOS ARM64 和无 PyTorch 的 Linux ARM64 各完成 76 组回比。新增 CLIP、法线和 OCR 识别 logits；媒体 E2E 与编辑器验收分开，Windows/x86 尚未验证。未接回产品执行路径，不改变下面的生产依赖数量。
