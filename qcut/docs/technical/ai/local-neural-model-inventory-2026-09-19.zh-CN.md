...
**2026-09-20 补充**：Filter 清单里四个不在 N 表、此前判为"密文 BM 容器"的神经文件已恢复并逐位对拍——`nodehub_c3_300_ilasdk`（C3 分类，`B` 图 174 层）、`tt_matting_large_v3.0`（`B` 图 140 层）、`tt_matting_v15.0`（int8 图 99 层，含膨胀深度卷积）、`tt_matting_relight_v1.0`（fp32 图 136 层，最大绝对误差 1.3e-5）。方法是把 BM 容器直接喂给 ByteNN 公开的 `EngineFactory::Create` + `Init(Config)`，运行库自行解密，再由捕获器从堆里取图与 arena（[记录](../../../research/local-model-pytorch/bytenn-init-parity.zh-CN.md)）。同日稍后，SMASH 自带 AES 包装的包也打开了一部分：`liblens` 导出厂商自己的读取器 `smash::package::ModelPackage`，只要带上该家族的包密钥即可解包，而密钥可以用 dyld interpose 在无头宿主里由 SDK 自己交出（[记录](../../../research/local-model-pytorch/smash-package-parity.zh-CN.md)）。共取得 7 把密钥、打开 6 个包、恢复 12 张网络并两种子逐位对拍：`tt_faceverify`、`tt_skeleton_v9.2`（multi/single）、`tt_skeletonlockon`（multi/single，其中 single 与人体包同图）、`tt_after_effect`（base/meaningless/portrait 三张质量评分网络）、`tt_matting_video_v1.2`（包内还套一层 BM v6，由 ByteNN 自解，288×288 fp32，逐层最大 `2.0e-3`，差值全部来自两类 softmax 的硬件倒数估计）。`tt_matting_relight`、`tt_matting_video` 与 facefitting_3d 的 MLP 已导出 ONNX 并用 onnxruntime 回比冻结原生输出；定点网络不导出，标准 ONNX 算子表达不了它们的舍入与回绕规则。同日再补真实素材证据：录下无头宿主真实渲染时 espresso 网络实际消费的输入与 SDK 取走的输出，3 个特效包 23 次推理、7 张网络全部一致；人脸检测器的前处理（1280×720 → GPU 2 倍缩小 .5 进位 → 7 位权重、每趟截断的可分离双线性 → BGR − 128 存 int8）逐像素复现，从原帧到 6 个检测输出逐位重现（[记录](../../../research/local-model-pytorch/espresso-real-input-parity.zh-CN.md)）。对齐网/freid 的裁剪几何与走 ByteNNEngine 的网络（皮肤分割、人体、抠像）尚未录到真实输入。`tt_face_attribute_age/exp` 的 `USTQ`/`F` 压缩权重由运行库在创建时展开，用创建后堆扫描取回展开 arena，两张网络也已逐位对拍（共 11 张）；`tt_face_extra_fast` 已恢复（不需要密钥：`face` 节点置 `face_extra_fast_model_name` 与 `face_fast_mode` 后由 face 算法加载，图走 espresso 路径直接捕获）；`tt_face_attribute_extra` 仍缺密钥——属性算法在这个 SDK 版本里只请求 age 一个模型；`tt_body_detection_lockon` 不是密钥问题——SDK 自己加载也返回 -4，该包与当前 SDK 版本不匹配。音频容器与脚本/数据文件仍不在恢复范围。
