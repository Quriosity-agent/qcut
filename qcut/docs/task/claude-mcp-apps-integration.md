...
When QCut opens, a Claude Code terminal is open by default (next to Remotion panels). Claude Code runs with an MCP server that can generate **interactive HTML UIs** — buttons for image ratio, video length, etc. After configuration, a button sends the settings to QCut for processing.
...
│ (spawned in PTY)    │  stdio  │ (Node.js, runs in Electron)  │
│                     │         │                              │
│ User talks to       │         │ Tools:                       │
...
Estimated total: ~45 minutes across 4 subtasks.
...
### Task 4: HTML App ↔ QCut Communication (~10 min)

**What**: The interactive HTML UIs need to send configuration back to QCut via the HTTP API.
...
- `electron/claude/claude-http-server.ts` (321 lines)
  - May need CORS headers for iframe-originated requests:
  ```javascript
...
**Acceptance criteria**:
- Clicking "Apply" in the HTML UI updates QCut project settings
...
- [ ] In Claude Code, run `/qcut` or ask "configure media settings" → interactive HTML appears
- [ ] Click aspect ratio button (e.g., 9:16) → "Apply" → QCut preview updates to 9:16
