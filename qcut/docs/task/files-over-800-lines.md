...
|------|-------|----------------|
| Components | 10 | timeline-track.tsx (1327) |
| Stores | 4 | remotion-store.ts (918) |
| Lib/Utils | 4 | claude-timeline-bridge-helpers.ts (840) |
| Electron | 5 | preload-integrations.ts (826) |
...
| 1013 | `electron/preload-types/electron-api.ts` | Split by API domain |
...
| 814 | `electron/utility/utility-bridge.ts` | Extract bridge categories |
...
1. **timeline-track.tsx** (1327 lines) — Complex UI component, high change frequency
2. **preview-panel.tsx** (1296 lines) — Large component with mixed concerns
3. **session-manager.ts** (1249 lines) — Core qagent module, split lifecycle vs execution
4. **word-timeline-view.tsx** (1166 lines) — Extract word list, timing editor
5. **drawing-canvas.tsx** (1136 lines) — Extract tool handlers, rendering logic

## Progress

- **Previous total**: 49 files over 800 lines
- **Current total**: 40 files over 800 lines
- **Resolved**: 9 files refactored below the limit (including the top offender `claude-timeline-bridge.ts` from 1559 → 708)
