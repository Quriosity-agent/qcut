...
The subsequent [UNORM precision correction](../../docs/task/jianying-filter-runtime-research/soft-glow-unorm-precision-2026-09-07.zh.md) fixes float double rounding and implements the provider output blend with byte inputs and binary64 arithmetic. The original CGL conversion oracle matches 666,580 channels with zero differences; its old float-product negative control differs 127 times. Six fixed native references now give RGB MAE 0.006529–0.050648, maximum error 6. This intentionally changes historical C++ output hashes; the report also records both improvements and regressions against existing UI export frames.
