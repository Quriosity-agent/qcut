...
2026-09-07 精度更新：[UNORM 与宿主字节混合报告](../../docs/task/jianying-filter-runtime-research/soft-glow-unorm-precision-2026-09-07.zh.md)。以下原 UI/视频观测保留历史源码身份；当前量化修正有意改变部分像素，不沿用旧输出 SHA 一致声明。
...
| --- | --- | --- | --- | --- | --- |
| `output-mix`（默认） | 0.84 | 2.4 | 0.8 | `round(I8+(F8−I8)×t)/255` | 逐字节I |
| `ui-snapshot`，t≤0.8 | `1−0.175t` | `3t` | `0.8t` | F | B |
| `ui-snapshot`，t>0.8 | 0.84 | 2.4 | `0.8t` | F | 不适用 |

I为原图，F为最终Normal输出，Q为RGBA8量化；I8/F8是对应字节，末端混合先做 binary64 乘法再加法和取整，非满强度保留 I 的 alpha。阈值比较包含0.8边界：80%为threshold=0.86／brightness=2.4，81%为0.84／2.4；保留这个分支变化，不做插值平滑。37%为0.93525／1.11，LUT opacity=0.296。
...
以下为 09-06 历史构建结果；后续量化修正的复测见文首链接。旧D634 CGL宿主使用完整输出混合，已有三输入×100%／37%的六组对照。新默认CLI重放六组，输出SHA全部与旧C++结果相同。原生参考、旧输出和旧源文件hash保持原样，旧hash是当时快照，不是当前源代码清单。
