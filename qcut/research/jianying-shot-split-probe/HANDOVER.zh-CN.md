...
| 边界说明 | 文件头 `+0x14` 的 2,307,596 字节 = 576,899 个 float;`GetWeightLen()` 报 2,307,592 = 实际用到的 576,898 个;差的 4 字节是权重区末尾一个未使用的 float,切片时以 576,898 为界 | 2911 + 303,236×4 = 1,215,855 到文件尾 72 字节是尾部元数据;最后一个 float 不属于任何层 |
