...
`weight-dump.mm` 现在把每层的卷积参数一并导出到 `params.tsv`,字段偏移都已核对:

| 层对象偏移 | 含义 | 核对依据 |
|---|---|---|
| +0x118 | 卷积核 (kh, kw) | 首层 3×3 |
| +0x120 | 步长 | 首层 (2,2),1×1 卷积 (1,1) |
| +0x128 / +0x130 | padding 起止 | 3×3 卷积为 1,1×1 卷积为 0 |
| +0x138 | 膨胀 | 恒为 (1,1) |
| +0x140 | 通道 (in, out) | 首层 3→8 |
| +0x158 | 输入尺寸 | 首层 96×96 |

**图的连接**:层对象 +0x190 与 +0x1c0 的字符串对卷积层就是它自己的名字,拿不到连线。但层名是 PyTorch 模块路径
(`features.N.conv.0.primary_conv.0` / `cheap_operation.0` / `conv.1.0` / `shortcut.*`),GhostNet 的拓扑是公开的,
按命名足以还原,最终靠与桥接的切点对齐来验证。

**预处理还没确定**。静态找过常见归一化常数,libcccreator 里 0x2d48d20 附近有 0.485/0.456/0.229/0.224 这样的
ImageNet 风格数值,但不成完整的均值方差表,也没找到来自镜头分割代码的引用,不能当结论。
建议改用实测法确定:给桥接加一条"输出 backbone 每帧 128 维特征"的通路,同一帧分别喂桥接和自己的实现,
换几种候选预处理(除以 255、除以 127.5 再减 1、ImageNet 归一化)看哪种能对上特征,一次就能定死。
