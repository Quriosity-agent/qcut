...
**注意:文件里的张量顺序不等于层顺序**,不要按层序顺推。试过用每层期望尺寸在段里顺序锁定,前 38 个能锁上
(其中 30 个间隔为 0,张量确实是紧挨着排的),但只覆盖 9,120 个参数就滑脱,而且锁到 `primary_conv` 位置上的数值明显偏大
(1x1 卷积不该有 13 这种值),而 `cheap_operation` 深度卷积位置上的值又完全正常。这说明顺序假设错了,不是锁定算法的问题。

正确做法是**先解析记录结构再按尺寸匹配**:权重段里每个张量前有很短的记录头(层间间隔统计:大量 0,其余 136/144/192/264 字节),
段首还有约 16KB 的头部区(首个张量在 +0x3e79)。把记录头解出来(多半带名字或长度字段),就能直接得到张量清单,
再用 `params.tsv` 的形状按尺寸对上层。

对上之后就能喂进 PyTorch 或 ONNX 复现前向,用同一段视频和桥接的切点做对齐验证。
