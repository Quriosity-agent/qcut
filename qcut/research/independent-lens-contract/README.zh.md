...
这里是 QCut 自行编写的 C++20 库、命令行工具和测试。默认构建只依赖 C++ 标准库，不加载剪映、OpenCV、模型或 GPU 程序。当前恢复了 `liblens` 中 `LENS::ALGORITHM::MoveSys::Util` 的六个 CPU 数值原语，以及 `smash::module::fsnew::base` 的 RGBA 仿射图像采样与 BGR 转换。六个数值原语是 3×3 矩阵乘法、求逆、点旋转、刚性点变换、高斯核、轨迹高斯平滑。
...
详细语义、身份、验证数字与未完成项见 [数值原语研究记录](../../docs/task/jianying-filter-runtime-research/lens-cpp-contract-2026-09-07.zh.md) 和 [图像仿射研究记录](../../docs/task/jianying-filter-runtime-research/lens-image-warp-2026-09-07.zh.md)。
...
只有显式开启 `LENS_CONTRACT_NATIVE_ORACLE=ON` 才编译 [数值诊断](native_oracle.cpp) 和 [图像诊断](image_warp_native_oracle.cpp)。工具限 macOS arm64；共用 [身份校验](native_identity.hpp)，在装载前检查完整文件 SHA256，解析每个导出符号后检查已加载 Mach-O UUID，不匹配即退出。图像诊断调用真实 `Mat` 构造和析构，额外检查导出锚点地址后才使用固定版本的内部入口。
