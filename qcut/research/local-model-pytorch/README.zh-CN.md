...
- [SMASH 加密模型包：让厂商自己的读取器解包](smash-package-parity.zh-CN.md)：`liblens` 导出的 `ModelPackage` 读取器带包密钥即可解包；密钥按家族不同、不在库字符串里，用 dyld interpose 在宿主里由 SDK 自己交出；共 7 把密钥、6 个包、9 张网络两种子逐位一致（含 `tt_after_effect` 三张与 `tt_matting_video` fp32 图）；人脸属性两张已解包但需 `USTQ` 权重解码，另两个包仍缺密钥，`tt_body_detection_lockon` 连 SDK 自己都加载失败。
