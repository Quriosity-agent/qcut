		...
		(
			...
			responseHeaders["Content-Security-Policy"] = [
				"default-src 'self' blob: data: app: https://cdn.tldraw.com; " +
					...
					"img-src 'self' blob: data: app: https://*.byteimg.com https://fal.run https://fal.media https://v3.fal.media https://v3b.fal.media https://api.iconify.design https://api.simplesvg.com https://api.unisvg.com https://avatars.githubusercontent.com https://i.ibb.co https://usercontent.jamendo.com https://cdn.tldraw.com https://lh3.googleusercontent.com https://kbrtxitvavpuimuihppz.supabase.co; " +
...
if (!isCliKeyCommand && !isHeadlessRecorder) {
	app.whenReady().then(async () => {
		...
		const handlers: [string, () => void | Promise<void>][] = [
			...
			[
				"JianyingPortraitAdjustmentIPC",
				() => {
					jianyingPortraitAdjustmentController =
						setupJianyingPortraitAdjustmentIPC({
							getMainWindow: () => mainWindow,
						});
				},
