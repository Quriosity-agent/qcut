 ...
 * from the verified private runtime snapshot without the Jianying app.
 */
...
export async function inspectJianyingShotSplit(): Promise<JianyingShotSplitStatus> {
	...
	try {
		...
		const bridgePath = await resolveJianyingShotSplitBridge({
			runtimeRoot,
			runtimeSha256: EXPECTED_SHOT_SPLIT_CORE_SHA256,
		});
		if (!bridgePath) {
			return { ...base, ...identity, message: BRIDGE_UNAVAILABLE_MESSAGE };
		}
		return {
			...base,
			...identity,
