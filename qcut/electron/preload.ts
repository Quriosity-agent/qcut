...
const electronAPI: ElectronAPI & Record<string, unknown> = {
	...
	jianyingEffects: {
		...
		download: (request) =>
			ipcRenderer.invoke(JIANYING_EFFECT_DOWNLOAD_CHANNEL, request),
		cover: (request) =>
			ipcRenderer.invoke(JIANYING_EFFECT_COVER_CHANNEL, request),
	},
	jianyingTransitions: {
	...
	jianyingFontLab: {
		list: (request) =>
			ipcRenderer.invoke(JIANYING_FONT_LAB_LIST_CHANNEL, request),
		load: (request) =>
	...
	jianyingTextRuntime: {
		inspect: (request) =>
			ipcRenderer.invoke(JIANYING_TEXT_RUNTIME_INSPECT_CHANNEL, request),
		render: (request) =>
			ipcRenderer.invoke(JIANYING_TEXT_RUNTIME_RENDER_CHANNEL, request),
		cancel: (request) =>
			ipcRenderer.invoke(JIANYING_TEXT_RUNTIME_CANCEL_CHANNEL, request),
	},
	jianyingEnvelope: {
