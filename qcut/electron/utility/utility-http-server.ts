	...
	return {
		webContents: {
			send(channel: string, ...args: any[]) {
				...
				requestFromMain("webcontents-send", { channel, args }).catch((err: Error) => {
					claudeLog.warn("HTTP", `Failed to send to renderer: ${err.message}`);
				});
			},
		},
	};
...
export function startUtilityHttpServer(config: UtilityHttpConfig): void {
	...
	async function requestSplitFromMain(elementId: string, splitTime: number, mode: string): Promise<any> {
	...
	// ==========================================================================
	router.get("/api/claude/media/:projectId", async (req) => listMediaFiles(req.params.projectId));
	...
	router.post("/api/claude/media/:projectId/import", async (req) => {
		if (!req.body?.source) throw new HttpError(400, "Missing 'source' in request body");
		const media = await importMediaFile(req.params.projectId, req.body.source);
		logOperation({ stage: 1, action: "import", details: `Imported media from path: ${req.body.source}`, timestamp: Date.now(), projectId: req.params.projectId });
		// Notify renderer via main process
		if (media) {
			requestFromMain("webcontents-send", {
				channel: "claude:media:imported",
				args: [{ path: media.path, name: media.name, id: media.id, type: media.type, size: media.size }],
			}).catch(() => { /* non-fatal */ });
		}
		return media;
	});
	...
	router.patch("/api/claude/media/:projectId/:mediaId/rename", async (req) => {
		if (!req.body?.newName) throw new HttpError(400, "Missing 'newName' in request body");
		return renameMediaFile(req.params.projectId, req.params.mediaId, req.body.newName);
	});

	router.post("/api/claude/media/:projectId/import-from-url", async (req) => {
		if (!req.body?.url) throw new HttpError(400, "Missing 'url' in request body");
		const result = await importMediaFromUrl(req.params.projectId, req.body.url, req.body.filename);
		logOperation({ stage: 1, action: "import", details: `Imported media from URL: ${req.body.url}`, timestamp: Date.now(), projectId: req.params.projectId });
		return result;
	});

	router.post("/api/claude/media/:projectId/batch-import", async (req) => {
		if (!Array.isArray(req.body?.items)) throw new HttpError(400, "Missing 'items' array in request body");
		const result = await batchImportMedia(req.params.projectId, req.body.items);
		logOperation({ stage: 1, action: "import", details: `Batch import processed ${req.body.items.length} item(s)`, timestamp: Date.now(), projectId: req.params.projectId });
		return result;
	});

	router.post("/api/claude/media/:projectId/:mediaId/extract-frame", async (req) => {
		if (typeof req.body?.timestamp !== "number") throw new HttpError(400, "Missing 'timestamp' (number) in request body");
		return extractFrame(req.params.projectId, req.params.mediaId, req.body.timestamp, req.body.format);
	});
	...
	router.get("/api/claude/timeline/:projectId", async (req) => {
		const timeline = await Promise.race([
			...
			new Promise<never>((_, reject) => setTimeout(() => reject(new HttpError(504, "Renderer timed out")), 5000)),
		]);
	...
	router.post("/api/claude/timeline/:projectId/import", async (req) => {
		if (!req.body?.data) throw new HttpError(400, "Missing 'data' in request body");
		const format = req.body.format || "json";
		let timeline;
		if (format === "md") {
			try { timeline = markdownToTimeline(req.body.data); }
			catch (e) { throw new HttpError(400, e instanceof Error ? e.message : "Invalid markdown"); }
		} else {
			if (typeof req.body.data === "string") {
				try { timeline = JSON.parse(req.body.data); }
				catch { throw new HttpError(400, "Invalid JSON in 'data'"); }
			} else {
		...
		const win = getWindow();
		win.webContents.send("claude:timeline:apply", { timeline, replace: req.body.replace === true });
		return { imported: true };
	});

	router.post("/api/claude/timeline/:projectId/elements", async (req) => {
		if (!req.body) throw new HttpError(400, "Missing element data in request body");
		const win = getWindow();
		const elementId = req.body.id || `element_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
		win.webContents.send("claude:timeline:addElement", { ...req.body, id: elementId });
		return { elementId };
	});

	router.post("/api/claude/timeline/:projectId/elements/batch", async (req) => {
		if (!Array.isArray(req.body?.elements)) throw new HttpError(400, "Missing 'elements' array in request body");
		// batchAddElements needs a BrowserWindow — proxy through main
		try {
			return await requestFromMain("batch-add-elements", { projectId: req.params.projectId, elements: req.body.elements });
		} catch (error) {
			throw new HttpError(400, error instanceof Error ? error.message : "Batch add failed");
		}
	});

	router.patch("/api/claude/timeline/:projectId/elements/batch", async (req) => {
		if (!Array.isArray(req.body?.updates)) throw new HttpError(400, "Missing 'updates' array in request body");
		try {
			return await requestFromMain("batch-update-elements", { updates: req.body.updates });
		} catch (error) {
			throw new HttpError(400, error instanceof Error ? error.message : "Batch update failed");
		}
	});

	router.patch("/api/claude/timeline/:projectId/elements/:elementId", async (req) => {
		const win = getWindow();
		win.webContents.send("claude:timeline:updateElement", { elementId: req.params.elementId, changes: req.body || {} });
		return { updated: true };
	});

	router.delete("/api/claude/timeline/:projectId/elements/batch", async (req) => {
		if (!Array.isArray(req.body?.elements)) throw new HttpError(400, "Missing 'elements' array in request body");
		try {
			return await requestFromMain("batch-delete-elements", { elements: req.body.elements, ripple: Boolean(req.body.ripple) });
		} catch (error) {
			throw new HttpError(400, error instanceof Error ? error.message : "Batch delete failed");
		}
	});

	router.delete("/api/claude/timeline/:projectId/elements/:elementId", async (req) => {
		const win = getWindow();
	...
	router.post("/api/claude/timeline/:projectId/arrange", async (req) => {
		if (!req.body?.trackId || typeof req.body.trackId !== "string") throw new HttpError(400, "Missing 'trackId' in request body");
		if (!req.body?.mode || typeof req.body.mode !== "string") throw new HttpError(400, "Missing 'mode' in request body");
		if (!["sequential", "spaced", "manual"].includes(req.body.mode)) throw new HttpError(400, "Invalid mode. Use sequential, spaced, or manual");
		try {
			return await requestFromMain("arrange-timeline", {
				trackId: req.body.trackId, mode: req.body.mode, gap: req.body.gap, order: req.body.order, startOffset: req.body.startOffset,
			});
		} catch (error) {
			throw new HttpError(400, error instanceof Error ? error.message : "Arrange request failed");
		}
	});

	router.post("/api/claude/timeline/:projectId/elements/:elementId/split", async (req) => {
		if (typeof req.body?.splitTime !== "number") throw new HttpError(400, "Missing 'splitTime' (number) in request body");
		const mode = req.body.mode || "split";
		if (!["split", "keepLeft", "keepRight"].includes(mode)) throw new HttpError(400, "Invalid mode. Use 'split', 'keepLeft', or 'keepRight'");
		return requestSplitFromMain(req.params.elementId, req.body.splitTime, mode);
	});

	router.post("/api/claude/timeline/:projectId/elements/:elementId/move", async (req) => {
		if (!req.body?.toTrackId) throw new HttpError(400, "Missing 'toTrackId' in request body");
		const win = getWindow();
		win.webContents.send("claude:timeline:moveElement", {
			elementId: req.params.elementId, toTrackId: req.body.toTrackId, newStartTime: req.body.newStartTime,
		});
		return { moved: true };
	});
	...
	router.post("/api/claude/timeline/:projectId/selection", async (req) => {
		if (!Array.isArray(req.body?.elements)) throw new HttpError(400, "Missing 'elements' array in request body");
		const win = getWindow();
		win.webContents.send("claude:timeline:selectElements", { elements: req.body.elements });
	...
	// Project routes
	// ==========================================================================
	router.get("/api/claude/project/:projectId/settings", async (req) => getProjectSettings(req.params.projectId));
	...
	router.get("/api/claude/project/:projectId/summary", async (req) => {
		try {
			...
			const [mediaFiles, settings] = await Promise.all([listMediaFiles(req.params.projectId), getProjectSettings(req.params.projectId)]);
			const exportJobs = listExportJobs(req.params.projectId);
			return generateProjectSummary({ timeline, mediaFiles, exportJobs, settings });
		} catch (error) {
			if (error instanceof HttpError) throw error;
			const msg = error instanceof Error ? error.message : "Failed to generate summary";
			throw new HttpError(msg.includes("Failed to read project") ? 400 : 500, msg);
		}
	});
	...
	router.post("/api/claude/project/:projectId/report", async (req) => {
		try {
			...
			const [mediaFiles, settings] = await Promise.all([listMediaFiles(req.params.projectId), getProjectSettings(req.params.projectId)]);
			const exportJobs = listExportJobs(req.params.projectId);
			const summary = generateProjectSummary({ timeline, mediaFiles, exportJobs, settings });
			...
			const report = await generatePipelineReport({ steps, summary, saveToDisk, outputDir, projectId: req.params.projectId });
			if (req.body?.clearLog === true) clearOperationLog({ projectId: req.params.projectId });
			return report;
		} catch (error) {
			if (error instanceof HttpError) throw error;
			throw new HttpError(500, error instanceof Error ? error.message : "Failed to generate report");
		}
	});
	...
	router.get("/api/claude/export/:projectId/recommend/:target", async (req) => getExportRecommendation(req.params.target));
	...
	router.post("/api/claude/export/:projectId/start", async (req) => {
		try {
			...
			return await startExportJob({ projectId: req.params.projectId, request: req.body || {}, timeline, mediaFiles });
		} catch (error) {
			if (error instanceof HttpError) throw error;
			throw new HttpError(400, error instanceof Error ? error.message : "Failed to start export");
		}
	});

	router.get("/api/claude/export/:projectId/jobs/:jobId", async (req) => {
		const job = getExportJobStatus(req.params.jobId);
		if (!job || job.projectId !== req.params.projectId) throw new HttpError(404, `Job not found: ${req.params.jobId}`);
		return job;
	});

	router.get("/api/claude/export/:projectId/jobs", async (req) => listExportJobs(req.params.projectId));

	// ==========================================================================
	// Diagnostics
	// ==========================================================================
	router.post("/api/claude/diagnostics/analyze", async (req) => {
		if (!req.body?.message) throw new HttpError(400, "Missing 'message' in error report");
	...
	// PersonaPlex
	// ==========================================================================
	router.post("/api/claude/personaplex/generate", async (req) => generatePersonaPlex(req.body));

	// ==========================================================================
	// MCP app preview forwarding
	// ==========================================================================
	router.post("/api/claude/mcp/app", async (req) => {
		if (!req.body || typeof req.body.html !== "string" || !req.body.html.trim()) throw new HttpError(400, "Missing 'html' in request body");
		...
		try {
			...
			return { forwarded: false, error: err instanceof Error ? err.message : "Unknown error" };
		}
	});
	...
	server = createServer((req, res) => {
		...
		req.setTimeout(30_000, () => {
			...
			res.end(JSON.stringify({ success: false, error: "Request timeout", timestamp: Date.now() }));
		});
		if (!checkAuth(req)) {
			res.writeHead(401, { "Content-Type": "application/json" });
			res.end(JSON.stringify({ success: false, error: "Unauthorized", timestamp: Date.now() }));
			return;
		}
	...
	server.listen(port, "127.0.0.1", () => {
		claudeLog.info("HTTP", `Server started on http://127.0.0.1:${port} (utility process)`);
	});
