...
describe("list", () => {
	...
	it("updates lastActivityAt when detection timestamp is newer", async () => {
		...
		const agentWithTimestamp: Agent = {
			...
			getActivityState: vi
				.fn()
				.mockResolvedValue({ state: "active", timestamp: newerTimestamp }),
		};
	...
	it("does not downgrade lastActivityAt when detection timestamp is older", async () => {
		...
		const agentWithOldTimestamp: Agent = {
			...
			getActivityState: vi
				.fn()
				.mockResolvedValue({ state: "active", timestamp: olderTimestamp }),
		};
...
describe("restore", () => {
	...
	it("uses getRestoreCommand when available", async () => {
		...
		const createCall = (mockRuntime.create as ReturnType<typeof vi.fn>).mock
			.calls[0][0];
