...
describe("Project ID Generation", () => {
	it("extracts basename correctly", () => {
		expect(generateProjectId("/Users/alice/repos/integrator")).toBe(
			"integrator"
		);
...
describe("Path Construction", () => {
	...
	it("getArchiveDir returns {baseDir}/sessions/archive", () => {
		...
		expect(archiveDir).toMatch(
			/\.qagent\/[a-f0-9]{12}-integrator\/sessions\/archive$/
		);
	});
	...
	it("getOriginFilePath returns {baseDir}/.origin", () => {
		...
		expect(originPath).toMatch(/\.qagent\/[a-f0-9]{12}-integrator\/\.origin$/);
	});
...
describe("Hash Collision Probability", () => {
	it("documents expected collision rate", () => {
		...
		const uniqueValues = 2 ** bitsOfEntropy;
