...
export function createSessionManager(deps: SessionManagerDeps): SessionManager {
	...
	): { sessionName: string; projectId: string }[] {
	...
	async function spawn(spawnConfig: SessionSpawnConfig): Promise<Session> {
		...
		if (spawnConfig.issueId && plugins.tracker) {
			try {
				...
				resolvedIssue = await plugins.tracker.getIssue(
					spawnConfig.issueId,
					project
				);
			} catch (err) {
				...
				if (isIssueNotFoundError(err)) {
					...
					throw new Error(
						`Failed to fetch issue ${spawnConfig.issueId}: ${err}`,
						{ cause: err }
					);
				}
			}
		...
		if (spawnConfig.issueId && plugins.tracker) {
			try {
				const issue = await plugins.tracker.getIssue(
					spawnConfig.issueId,
					project
				);
	...
	async function send(sessionId: SessionId, message: string): Promise<void> {
		...
		if (raw.runtimeHandle) {
			...
			handle = {
				id: sessionId,
				runtimeName: config.defaults.runtime,
				data: {},
			};
		}
