...
describe("API Routes", () => {
	...
	describe("POST /api/sessions/:id/send", () => {
		...
		it("returns 404 for unknown session", async () => {
			...
			).mockRejectedValueOnce(new Error("Session nonexistent not found"));
	...
	describe("POST /api/sessions/:id/kill", () => {
		it("kills a valid session", async () => {
			const req = makeRequest("/api/sessions/backend-3/kill", {
				method: "POST",
			});
		...
		it("returns 404 for unknown session", async () => {
			(
				mockSessionManager.kill as ReturnType<typeof vi.fn>
			).mockRejectedValueOnce(new Error("Session nonexistent not found"));
			const req = makeRequest("/api/sessions/nonexistent/kill", {
				method: "POST",
			});
			const res = await killPOST(req, {
				params: Promise.resolve({ id: "nonexistent" }),
	...
	describe("POST /api/sessions/:id/restore", () => {
		it("restores a killed session", async () => {
			const req = makeRequest("/api/sessions/frontend-1/restore", {
				method: "POST",
			});
		...
		it("returns 404 for unknown session", async () => {
			const req = makeRequest("/api/sessions/nonexistent/restore", {
				method: "POST",
			});
			const res = await restorePOST(req, {
				params: Promise.resolve({ id: "nonexistent" }),
		...
		it("returns 409 for active session", async () => {
			const req = makeRequest("/api/sessions/backend-9/restore", {
				method: "POST",
			});
			const res = await restorePOST(req, {
	...
	describe("POST /api/prs/:id/merge", () => {
		...
		it("returns 422 for non-mergeable PR", async () => {
			(
				mockSCM.getMergeability as ReturnType<typeof vi.fn>
		...
		it("returns 409 for merged PR", async () => {
			(mockSCM.getPRState as ReturnType<typeof vi.fn>).mockResolvedValueOnce(
				"merged"
			);
			const req = makeRequest("/api/prs/432/merge", { method: "POST" });
			const res = await mergePOST(req, {
