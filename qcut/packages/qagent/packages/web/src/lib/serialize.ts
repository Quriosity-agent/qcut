...
export async function enrichSessionPR(
	...
	const results = await Promise.allSettled([
		scm.getPRSummary
			...
			: scm
					.getPRState(pr)
					.then((state) => ({ state, title: "", additions: 0, deletions: 0 })),
		scm.getCIChecks(pr),
		scm.getCISummary(pr),
	...
	if (
		mostFailed &&
		!dashboard.pr.mergeability.blockers.includes(
...
export async function enrichSessionsMetadata(
	...
	const projects = coreSessions.map((core) =>
		resolveProject(core, config.projects)
	);
