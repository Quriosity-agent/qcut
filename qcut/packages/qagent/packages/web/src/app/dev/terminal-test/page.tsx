...
function TerminalTestPageContent() {
	...
	const defaultNewSession =
		availableSessions[1] || availableSessions[0] || "ao-orchestrator";
				...
				<section className="mb-8 rounded-lg border border-[var(--color-border-default)] bg-[var(--color-bg-secondary)] p-6">
					...
					<div className="space-y-3 text-sm text-[var(--color-text-secondary)]">
						<p>
							...
							Browser clipboard (Cmd+C/Ctrl+C) only worked when an iTerm2 client
							was attached to the tmux session.
						</p>
						<p>
							<strong className="text-[var(--color-text-primary)]">
								Impact:
							</strong>{" "}
							Users had to keep iTerm2 tabs open in the background for clipboard
							to work in the web dashboard.
						</p>
						<p>
							<strong className="text-[var(--color-text-primary)]">
								Investigation time:
							</strong>{" "}
							12+ hours of debugging across tmux, ttyd, xterm.js, and macOS
							clipboard systems.
						</p>
					</div>
				</section>
					...
					<div className="space-y-4 text-sm">
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								<li>
									tmux uses OSC 52 escape sequences to synchronize clipboard
									with terminals
								</li>
								<li>
									Format:{" "}
								...
								<li>
									Terminal must support OSC 52 and have proper capabilities
									declared
								</li>
							</ul>
						</div>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								<li>
									tmux queries terminal capabilities using Device Attributes
									(DA/XDA)
								</li>
								<li>
									XDA query:{" "}
								...
								<li>
									Terminal responds with identification string containing
									terminal type (e.g., "XTerm(370)", "iTerm2 ")
								</li>
								<li>
									Based on response, tmux enables features like TTYC_MS
									(clipboard support)
								</li>
							</ul>
						</div>
							...
							<div className="rounded-lg border border-[var(--color-accent-red)] bg-[var(--color-bg-tertiary)] p-4">
								...
								<ul className="ml-6 mt-2 list-disc space-y-1 text-[var(--color-text-secondary)]">
									<li>
										...
										<code className="rounded bg-black px-1 py-0.5">
											test.skip('CSI &gt; Ps q - Report xterm name and version
											(XTVERSION)')
										</code>
									</li>
									<li>
										Without XDA response, tmux doesn't recognize the terminal as
										clipboard-capable
									</li>
									<li>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									OSC 52 sequences are then sent to ALL clients, including
									browser (ttyd/xterm.js)
								</li>
								<li>
									This is why clipboard "magically worked" when iTerm2 was
									attached
								</li>
							</ul>
						</div>
					...
					<div className="space-y-4 text-sm">
						<div>
							...
							<p className="mb-2 text-[var(--color-text-secondary)]">
								Created custom terminal component that registers an XDA handler
								using xterm.js parser API:
							</p>
    ...
    return true;
						...
						<div>
							...
							<ol className="ml-6 list-decimal space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									tmux detects "XTerm(" in response and enables TTYC_MS
									capability
								</li>
								<li>
									OSC 52 sequences now flow: tmux → WebSocket → xterm.js →
									navigator.clipboard
								</li>
								<li>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									<strong>New:</strong> Browser → Custom WebSocket (node-pty) →
									tmux
								</li>
								<li>
					...
					<div className="space-y-3 text-sm">
						<div className="rounded-lg border border-[var(--color-accent-orange)] bg-[var(--color-bg-tertiary)] p-4">
							<p className="font-semibold text-[var(--color-accent-orange)]">
								CRITICAL: This implementation requires Node 20.x (currently
								20.20.0)
							</p>
						</div>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								...
								<li>
									Root cause: node-pty's native module (darwin-arm64 prebuild)
									fails to spawn processes on Node 25
								</li>
								<li>
						...
						<div>
							...
							<ul className="ml-6 list-disc space-y-1 text-[var(--color-text-secondary)]">
								<li>
									<strong>Option 1:</strong> Wait for node-pty 1.2.0 stable
									release with Node 25+ support
								</li>
								<li>
									<strong>Option 2:</strong> Test node-pty beta versions
									(currently 1.2.0-beta.11)
								</li>
								<li>
									<strong>Option 3:</strong> Switch to alternative PTY library
									(e.g., xterm-pty, node-child-pty)
								</li>
							</ul>
						</div>
						...
						<div className="rounded-lg bg-[var(--color-bg-tertiary)] p-4">
							...
							<ol className="ml-6 list-decimal space-y-1 text-xs text-[var(--color-text-secondary)]">
								...
								<li>
									Test DirectTerminal: verify connection, clipboard, resize
								</li>
				...
				<div className="mb-4">
					...
					{oldSessionId === newSessionId && (
						<div className="mt-2 rounded border border-[var(--color-accent-orange)] bg-[var(--color-bg-tertiary)] p-2 text-xs text-[var(--color-text-secondary)]">
							⚠️ Using same session for both terminals. To avoid port conflicts,
					...
					<section className="mb-8">
						...
						<div className="mt-4 rounded-lg border border-[var(--color-accent-blue)] bg-[var(--color-bg-secondary)] p-4">
							...
							<ol className="ml-6 list-decimal space-y-1 text-sm text-[var(--color-text-secondary)]">
								...
								<li>
									<strong className="text-[var(--color-text-primary)]">
										Expected: RIGHT terminal works, LEFT may not (unless iTerm2
										is attached)
									</strong>
								</li>
							</ol>
					...
					<div className="space-y-4 text-sm text-[var(--color-text-secondary)]">
						...
						<div>
							...
							<ol className="ml-6 list-decimal space-y-2">
								<li>
									<strong>Suspected ttyd clipboard handling</strong> - Spent
									hours analyzing ttyd source code, checking OSC 52 passthrough.
									ttyd was innocent - it passes escape sequences correctly.
								</li>
								<li>
									<strong>Blamed xterm.js configuration</strong> - Tried every
									possible xterm.js option, clipboard addon configurations,
									terminal type settings. None made a difference.
								</li>
								<li>
									<strong>Investigated macOS clipboard permissions</strong> -
									Checked browser permissions, sandbox attributes,
									navigator.clipboard API. All were correct.
								</li>
								<li>
									<strong>Suspected WebSocket encoding issues</strong> - Checked
									binary vs text mode, UTF-8 encoding, base64 handling. All
									correct.
								</li>
								<li>
									...
									in tmux.conf. Didn't help - tmux needs the terminal to declare
									support.
								</li>
							</ol>
						</div>
						...
						<div>
							...
							<div className="space-y-2">
								<p>
									<strong>What finally worked:</strong> Registering an XDA
									(Extended Device Attributes) handler in xterm.js using{" "}
									<code className="rounded bg-black px-1 py-0.5">
										terminal.parser.registerCsiHandler()
									</code>
								</p>
  ...
  () => {
    ...
    return true;
								...
								<p className="mt-2">
									This single handler made tmux recognize our terminal as
									clipboard-capable. Clipboard immediately started working.
								</p>
							</div>
						</div>
						...
						<div>
							...
							<ol className="ml-6 list-decimal space-y-2">
								<li>
									<strong>Deep-dive into tmux source code</strong> - Used
									DeepWiki.com to analyze tmux's terminal capability detection
									logic in{" "}
									<code className="rounded bg-black px-1 py-0.5">
										tty-keys.c
									</code>{" "}
								...
								<li>
									<strong>Discovered XDA queries</strong> - Found that tmux
									sends{" "}
									<code className="rounded bg-black px-1 py-0.5">
										CSI &gt; q
									</code>{" "}
									(XTVERSION) to detect terminal type
								</li>
								<li>
									<strong>Traced the "iTerm2 magic"</strong> - Realized why
									clipboard worked when iTerm2 was attached: iTerm2 responds to
									XDA queries, enabling clipboard for the entire tmux session
								</li>
								<li>
									<strong>Checked xterm.js implementation</strong> - Found that
									XDA is marked as TODO in xterm.js tests:{" "}
									<code className="rounded bg-black px-1 py-0.5">
										test.skip('CSI &gt; Ps q - Report xterm name and version
										(XTVERSION)')
									</code>
								...
								<li>
									<strong>Implemented custom handler</strong> - Used xterm.js
									parser API to register our own XDA handler
								</li>
							</ol>
						</div>
						...
						<div className="rounded-lg border border-[var(--color-accent-orange)] bg-[var(--color-bg-tertiary)] p-4">
							...
							<ol className="ml-6 list-decimal space-y-2 text-[var(--color-text-secondary)]">
								<li>
									<strong>Start with tmux source code first</strong> - Instead
									of debugging xterm.js and ttyd, we should have immediately
									checked how tmux detects terminal capabilities
								</li>
								<li>
									...
									or using a terminal protocol analyzer would have revealed the
									XDA queries being sent
								</li>
								<li>
									<strong>Compare working vs broken scenarios</strong> - Should
									have captured and diffed the escape sequences when iTerm2 was
									attached vs not attached earlier
								</li>
								<li>
									<strong>Check xterm.js issues/limitations first</strong> - A
									GitHub search for "xterm.js XDA" or "xterm.js device
									attributes" would have revealed it's unimplemented
								</li>
								<li>
									<strong>Read tmux documentation on terminal types</strong> -
									tmux man page mentions terminal capability detection, but we
									focused on clipboard settings instead
								</li>
							</ol>
						</div>
				...
				<div className="mt-8 border-t border-[var(--color-border-default)] pt-4 text-center text-xs text-[var(--color-text-secondary)]">
					<p>
						Investigation: Feb 15-16, 2026 • Duration: 12+ hours • Status: ✅
						Resolved
					</p>
