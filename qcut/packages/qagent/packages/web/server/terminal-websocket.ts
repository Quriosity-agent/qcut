...
function getOrSpawnTtyd(
	...
	if (availablePorts.size > 0) {
		...
		if (nextPort >= MAX_PORT) {
			throw new Error(
				`Port exhaustion: reached maximum of ${MAX_PORT - 7800} terminal instances`
			);
		}
		port = nextPort++;
	}
	...
	console.log(
		`[Terminal] Spawning ttyd for ${tmuxSessionName} on port ${port}`
	);
	...
	mouseProc.on("error", (err) => {
		console.error(
			`[Terminal] Failed to set mouse mode for ${tmuxSessionName}:`,
			err.message
		);
	});
	...
	statusProc.on("error", (err) => {
		console.error(
			`[Terminal] Failed to hide status bar for ${tmuxSessionName}:`,
			err.message
		);
	});
...
const server = createServer(async (req, res) => {
	...
	if (url.pathname === "/terminal") {
		...
		try {
			...
			console.error(
				`[Terminal] Failed to start terminal for ${sessionId}:`,
				errorMsg
			);
