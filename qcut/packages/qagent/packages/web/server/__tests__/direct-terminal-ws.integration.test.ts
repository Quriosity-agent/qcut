...
beforeAll(() => {
	...
	execFileSync(
		TMUX,
		["new-session", "-d", "-s", TEST_SESSION, "-x", "80", "-y", "24"],
		{ timeout: 5000 }
	);
	execFileSync(
		TMUX,
		["new-session", "-d", "-s", TEST_HASH_SESSION, "-x", "80", "-y", "24"],
		{ timeout: 5000 }
	);
...
afterAll(() => {
	...
	try {
		execFileSync(TMUX, ["kill-session", "-t", TEST_HASH_SESSION], {
			timeout: 5000,
		});
	} catch {
		/* already dead */
...
describe("health endpoint", () => {
	...
	it("health active count matches number of connections", async () => {
		...
		execFileSync(
			TMUX,
			["new-session", "-d", "-s", secondSession, "-x", "80", "-y", "24"],
			{ timeout: 5000 }
		);
		...
		try {
			...
			try {
				execFileSync(TMUX, ["kill-session", "-t", secondSession], {
					timeout: 5000,
				});
			} catch {
				/* */
...
describe("WebSocket connection validation", () => {
	...
	it("rejects connection with path traversal in session ID", async () => {
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=../../../etc/passwd`
		);
		const result = await waitForWsClose(ws);
	...
	it("rejects connection with shell injection in session ID", async () => {
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=test;rm%20-rf%20/`
		);
		const result = await waitForWsClose(ws);
	...
	it("rejects pipe in session ID", async () => {
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=test|cat%20/etc/passwd`
		);
		const result = await waitForWsClose(ws);
	...
	it("rejects connection for nonexistent tmux session", async () => {
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=ao-nonexistent-999`
		);
		const result = await waitForWsClose(ws);
	...
	it("rejects connection for session that doesn't exist in tmux", async () => {
		...
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=definitely-not-real-${Date.now()}`
		);
		const result = await waitForWsClose(ws);
...
describe("hash-prefixed session resolution", () => {
	it("resolves hash-prefixed tmux session by suffix match", async () => {
		...
		execFileSync(
			TMUX,
			["new-session", "-d", "-s", hashPrefixedName, "-x", "80", "-y", "24"],
			{ timeout: 5000 }
		);
		...
		try {
			...
			try {
				execFileSync(TMUX, ["kill-session", "-t", hashPrefixedName], {
					timeout: 5000,
				});
			} catch {
				/* */
	...
	it("does NOT cross-match ao-1 to hash-ao-15 via prefix", async () => {
		...
		execFileSync(
			TMUX,
			["new-session", "-d", "-s", hashSession15, "-x", "80", "-y", "24"],
			{ timeout: 5000 }
		);
		...
		try {
			...
			try {
				execFileSync(TMUX, ["kill-session", "-t", hashSession15], {
					timeout: 5000,
				});
			} catch {
				/* */
...
describe("connection lifecycle", () => {
	it("cleans up activeSessions on WebSocket close", async () => {
		...
		execFileSync(
			TMUX,
			["new-session", "-d", "-s", cleanupSession, "-x", "80", "-y", "24"],
			{ timeout: 5000 }
		);
		...
		try {
			...
			try {
				execFileSync(TMUX, ["kill-session", "-t", cleanupSession], {
					timeout: 5000,
				});
			} catch {
				/* */
	...
	it("server stays healthy after connection errors", async () => {
		...
		const ws = new WebSocket(
			`ws://localhost:${port}/ws?session=nonexistent-${Date.now()}`
		);
