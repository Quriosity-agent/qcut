...
describe("findTmux", () => {
	...
	it("checks all three standard locations in order", () => {
		...
		expect(mockExec).toHaveBeenNthCalledWith(2, "/usr/local/bin/tmux", ["-V"], {
			timeout: 5000,
		});
		expect(mockExec).toHaveBeenNthCalledWith(3, "/usr/bin/tmux", ["-V"], {
			timeout: 5000,
		});
	});
...
describe("resolveTmuxSession", () => {
	...
	describe("hash-prefix resolution", () => {
		...
		it("does NOT match ambiguous suffixes (bugbot: hash-my-app-1 vs app-1)", () => {
			...
			expect(resolveTmuxSession("app-1", TMUX, mockExec)).toBe(
				"8474d6f29887-app-1"
			);
		});
		...
		it("only matches valid 12-char lowercase hex prefix", () => {
			...
			expect(resolveTmuxSession("ao-15", TMUX, mockExec)).toBe(
				"abcdef012345-ao-15"
			);
		});
		...
		it("matches ao-1 to hash-ao-1 (not hash-ao-15)", () => {
			...
			expect(resolveTmuxSession("ao-1", TMUX, mockExec)).toBe(
				"aabbccddeef0-ao-1"
			);
		});
		...
		it("matches session with underscores", () => {
			...
			expect(resolveTmuxSession("my_session", TMUX, mockExec)).toBe(
	...
	describe("edge cases", () => {
		it("handles session name that looks like a hash (all hex chars)", () => {
			...
			expect(resolveTmuxSession("abcdef123456", TMUX, mockExec)).toBe(
		...
		it("handles very long session list", () => {
			...
			expect(resolveTmuxSession("session-50", TMUX, mockExec)).toBe(
				`${hex50}-session-50`
			);
		});
		...
		it("handles session list where target is last entry", () => {
			...
			expect(resolveTmuxSession("ao-target", TMUX, mockExec)).toBe(
				"a0b1c2d3e4f5-ao-target"
			);
		});
