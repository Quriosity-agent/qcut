...
async function detectEnvironment(workingDir: string): Promise<EnvironmentInfo> {
	...
	const isGitRepo =
		(await git(["rev-parse", "--git-dir"], workingDir)) !== null;
	...
	const currentBranch = isGitRepo
		? await git(["branch", "--show-current"], workingDir)
		: null;

	// Detect the actual default branch (main/master/next)
	const defaultBranch = isGitRepo
		? await detectDefaultBranch(workingDir, ownerRepo)
		: null;
...
export function registerInit(program: Command): void {
	program
		...
		.action(
			async (opts: { output: string; auto?: boolean; smart?: boolean }) => {
				...
				try {
					...
					const checks = [
						{
							name: "Git",
							pass: (await execSilent("git", ["--version"])) !== null,
						},
					...
					console.log(
						chalk.dim("See SETUP.md for detailed configuration options.\n")
					);

					if (!env.hasTmux) {
						console.log(
							chalk.yellow("Note: tmux is required for the default runtime.")
						);
						console.log(chalk.dim("Install with: brew install tmux\n"));
					}
					...
					if (!env.ghAuthed && env.hasGh) {
						console.log(
							chalk.yellow(
								"Note: Authenticate GitHub CLI for full functionality."
							)
						);
						console.log(chalk.dim("Run: gh auth login\n"));
					}
...
async function handleAutoMode(
	...
	if (smart) {
		...
		console.log(
			chalk.dim("  🚀 Auto-generating config with smart defaults...\n")
		);
	}
	...
	if (smart) {
		...
		console.log(
			chalk.yellow("  ⚠ AI-powered rule generation not yet implemented")
		);
	...
	if (hasPlaceholderRepo) {
		...
		console.log(
			chalk.dim(
				"  Update the 'repo' field in the config before spawning agents\n"
			)
		);
	}
	...
	if (!env.hasTmux) {
		console.log(
			chalk.yellow("⚠ tmux not found - install with: brew install tmux")
		);
	}
	if (!env.ghAuthed && env.hasGh) {
		console.log(
			chalk.yellow("⚠ GitHub CLI not authenticated - run: gh auth login")
		);
	}
