...
export function registerSpawn(program: Command): void {
	program
		...
		.argument(
			"[issue]",
			"Issue identifier (e.g. INT-1234, #42) - must exist in tracker"
		)
		.option("--open", "Open session in terminal tab")
		.option(
			"--agent <name>",
			"Override the agent plugin (e.g. codex, claude-code)"
		)
		.action(
			async (
				projectId: string,
...
export function registerBatchSpawn(program: Command): void {
	program
		...
		.action(
			async (projectId: string, issues: string[], opts: { open?: boolean }) => {
				const config = loadConfig();
				if (!config.projects[projectId]) {
					console.error(
						chalk.red(
				...
				for (const issue of issues) {
					...
					if (spawnedIssues.has(issue.toLowerCase())) {
						console.log(
							chalk.yellow(`  Skip ${issue} — duplicate in this batch`)
						);
					...
					if (existingSessionId) {
						console.log(
							chalk.yellow(
								`  Skip ${issue} — already has session: ${existingSessionId}`
							)
						);
