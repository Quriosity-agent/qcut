...
export function registerDashboard(program: Command): void {
	program
		...
		.action(
			async (opts: { port?: string; open?: boolean; rebuild?: boolean }) => {
				...
				if (opts.rebuild) {
					...
					const runningWebDir = runningPid
						? await findProcessWebDir(runningPid)
						: null;
					...
					if (runningPid) {
						...
						console.log(
							chalk.dim(
								`Stopping dashboard (PID ${runningPid}) on port ${port}...`
							)
						);
						try {
