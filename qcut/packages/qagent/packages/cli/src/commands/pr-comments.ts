...
export function registerPRComments(program: Command): void {
	...
	cmd
		...
		.action(
			async (
				...
				const args = [
					resolve(scriptsDir(), "resolve-thread.sh"),
					repo,
					pr,
					commentId,
				];
	...
	cmd
		.command("forward")
		.description(
			"Export comments and send structured message to an agent session"
		)
		...
		.action(async (sessionId: string) => {
			...
			if (!project?.repo) {
				console.error(
					chalk.red(`No repo configured for project ${session.projectId}`)
				);
				process.exit(1);
			}
