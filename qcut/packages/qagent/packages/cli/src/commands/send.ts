	...
	program
		...
		.action(
			async (
				...
				const timeoutMs =
					(isNaN(parsedTimeout) || parsedTimeout <= 0 ? 600 : parsedTimeout) *
					1000;
				...
				for (let attempt = 1; attempt <= 3; attempt++) {
					...
					if (hasQueuedMessage(output)) {
						console.log(
							chalk.green("Message queued (session finishing previous task)")
						);
						return;
					}
					if (attempt < 3) {
				...
				console.log(
					chalk.yellow("Message sent — could not confirm it was received")
				);
			}
		);
}
