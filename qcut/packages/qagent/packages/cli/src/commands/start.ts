				...
				try {
					...
					if (opts?.dashboard !== false) {
						...
						if (!existsSync(resolve(webDir, "package.json"))) {
							throw new Error(
								"Could not find @composio/ao-web package. Run: bun install"
							);
						}
						...
						console.log(
							chalk.dim("  (Dashboard will be ready in a few seconds)\n")
						);
					}
...
export function registerStop(program: Command): void {
	program
		...
		.action(async (projectArg?: string) => {
			try {
				const config = loadConfig();
				const { projectId: _projectId, project } = resolveProject(
					config,
					projectArg
				);
				const sessionId = `${project.sessionPrefix}-orchestrator`;
				...
				if (existing) {
					...
					console.log(
						chalk.yellow(`Orchestrator session "${sessionId}" is not running`)
					);
				}
