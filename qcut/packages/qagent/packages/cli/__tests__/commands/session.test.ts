...
describe("session kill", () => {
	it("rejects unknown session (no matching project)", async () => {
		mockSessionManager.kill.mockRejectedValue(
			new Error("Session not found: unknown-1")
		);
...
describe("session cleanup", () => {
	...
	it("dry run shows what would be cleaned without doing it", async () => {
		...
		expect(mockSessionManager.cleanup).toHaveBeenCalledWith(undefined, {
			dryRun: true,
		});
	});
