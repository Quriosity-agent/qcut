...
describe("cleanNextCache", () => {
	it("deletes .next directory when it exists", async () => {
		...
		mkdirSync(join(webDir, ".next", "server", "vendor-chunks"), {
			recursive: true,
		});
		writeFileSync(
		...
		const { cleanNextCache } = await import(
			"../../src/lib/dashboard-rebuild.js"
		);
...
describe("findRunningDashboardPid", () => {
	it("returns PID when a process is listening", async () => {
		...
		const { findRunningDashboardPid } = await import(
			"../../src/lib/dashboard-rebuild.js"
		);

		const pid = await findRunningDashboardPid(3000);
		expect(pid).toBe("12345");
		expect(mockExecSilent).toHaveBeenCalledWith("lsof", [
			"-ti",
			":3000",
			"-sTCP:LISTEN",
		]);
...
describe("findProcessWebDir", () => {
	it("extracts cwd from lsof output", async () => {
		...
		const { findProcessWebDir } = await import(
			"../../src/lib/dashboard-rebuild.js"
		);
