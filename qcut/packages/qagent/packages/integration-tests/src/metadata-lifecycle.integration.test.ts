...
describe("metadata lifecycle (real filesystem)", () => {
	...
	it("listMetadata returns session IDs, excluding archive directory", () => {
		...
		writeMetadata(sessionsDir, "session-a", {
			worktree: "/a",
			branch: "a",
			status: "idle",
		});
		writeMetadata(sessionsDir, "session-b", {
			worktree: "/b",
			branch: "b",
			status: "working",
		});
		writeMetadata(sessionsDir, "session-c", {
			worktree: "/c",
			branch: "c",
			status: "done",
		});
	...
	it("validates session ID rejects path traversal attempts", () => {
		...
		expect(() => readMetadata(sessionsDir, "../escape")).toThrow(
			"Invalid session ID"
		);
		expect(() => readMetadata(sessionsDir, "foo/bar")).toThrow(
			"Invalid session ID"
		);
		expect(() => readMetadata(sessionsDir, "foo bar")).toThrow(
			"Invalid session ID"
