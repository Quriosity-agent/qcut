...
describe.skipIf(!realProject)(
	"path encoding & JSONL reading (real Claude data)",
	() => {
		...
		it("readLastJsonlEntry parses a real session JSONL file", async () => {
			...
			expect(
				entry!.lastType === null || typeof entry!.lastType === "string"
			).toBe(true);
		});
...
describe.skipIf(!canRun)("agent-claude-code (integration)", () => {
	...
	it("getActivityState → returns valid non-exited state while agent is alive", () => {
		...
		expect([
			...
			"waiting_input",
			"blocked",
		]).toContain(aliveActivityState?.state ?? null);
	});
