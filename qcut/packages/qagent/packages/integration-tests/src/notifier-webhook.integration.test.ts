...
describe("notifier-webhook integration", () => {
	...
	describe("payload serialization", () => {
		...
		it("post includes message and context in payload", async () => {
			...
			await notifier.post!("All sessions done", {
	...
	describe("non-retryable errors", () => {
		it.each([
			400, 401, 403, 404,
		])("HTTP %d -> single fetch call, immediate rejection", async (status) => {
	...
	describe("config validation", () => {
		it("throws on non-http(s) URL", () => {
			expect(() => webhookPlugin.create({ url: "ftp://example.com" })).toThrow(
				"must be http(s)"
			);
		});
		...
		it("accepts http URL", () => {
			expect(() =>
				webhookPlugin.create({ url: "http://localhost:3000/hook" })
			).not.toThrow();
		});
