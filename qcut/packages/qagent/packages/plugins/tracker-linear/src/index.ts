						...
						settle(() => {
							try {
								...
								if (status < 200 || status >= 300) {
									reject(
										new Error(
											`Linear API returned HTTP ${status}: ${text.slice(0, 200)}`
										)
									);
									return;
								}
								...
								if (json.errors && json.errors.length > 0) {
									reject(
										new Error(`Linear API error: ${json.errors[0].message}`)
									);
									return;
								}
								if (!json.data) {
	...
	return async <T>(
		...
		try {
			...
			if (!result.successful) {
				throw new Error(
					`Composio Linear API error: ${result.error ?? "unknown error"}`
				);
			}
	...
	return {
		...
		async generatePrompt(
			...
			if (issue.priority !== undefined) {
				...
				lines.push(
					`Priority: ${priorityNames[issue.priority] ?? String(issue.priority)}`
				);
			}
          ...
          issue(id: $id) {
			...
			if (update.assignee) {
				const usersData = await query<{
					users: {
						nodes: Array<{ id: string; displayName: string; name: string }>;
					};
				}>(
            ...
            issue(id: $id) {
              labels { nodes { id } }
				...
				const existingIds = new Set(
					existingData.issue.labels.nodes.map((l) => l.id)
				);
            ...
            issueLabels(filter: { team: { id: { eq: $teamId } } }) {
              nodes { id name }
				...
				const labelMap = new Map(
					labelsData.issueLabels.nodes.map((l) => [l.name, l.id])
				);
