...
describe("tracker-linear plugin", () => {
	...
	describe("listIssues", () => {
		...
		it("passes state filter for open issues", async () => {
			...
			const writeCall =
				requestMock.mock.results[0].value.write.mock.calls[0][0];
	...
	describe("updateIssue", () => {
		...
		it("changes state to open (unstarted)", async () => {
			...
			const writeCall =
				requestMock.mock.results[2].value.write.mock.calls[0][0];
			const body = JSON.parse(writeCall);
		...
		it("adds a comment", async () => {
			...
			// Verify comment body
			const writeCall =
				requestMock.mock.results[1].value.write.mock.calls[0][0];
			const body = JSON.parse(writeCall);
		...
		it("updates assignee by resolving display name to ID", async () => {
			...
			mockLinearAPI({
				users: {
					nodes: [{ id: "user-1", displayName: "Alice", name: "Alice Smith" }],
				},
			});
		...
		it("updates labels additively (merges with existing)", async () => {
			...
			mockLinearAPI({
				issue: { labels: { nodes: [{ id: "label-existing" }] } },
			});
			// 3: team label lookup
			mockLinearAPI({
				issueLabels: {
			...
			const writeCall =
				requestMock.mock.results[3].value.write.mock.calls[0][0];
			const body = JSON.parse(writeCall);
