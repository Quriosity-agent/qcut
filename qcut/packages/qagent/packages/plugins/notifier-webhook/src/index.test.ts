...
describe("notifier-webhook", () => {
	...
	describe("create", () => {
		...
		it("throws on invalid URL scheme", () => {
			expect(() => create({ url: "ftp://example.com" })).toThrow(
				"must be http(s)"
			);
		});

		it("throws on javascript: URL", () => {
			expect(() => create({ url: "javascript:alert(1)" })).toThrow(
				"must be http(s)"
			);
		});
