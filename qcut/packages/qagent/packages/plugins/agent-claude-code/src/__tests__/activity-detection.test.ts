...
describe("Claude Code Activity Detection", () => {
	...
	describe("getActivityState", () => {
		...
		it("returns 'exited' when process is not running", async () => {
			...
			expect((await agent.getActivityState(makeSession()))?.state).toBe(
				"exited"
			);
		});
			...
			expect(
				(
					await agent.getActivityState(
						makeSession({ runtimeHandle: undefined })
					)
				)?.state
			).toBe("exited");
		});
		...
		it("returns 'exited' when runtimeHandle is null", async () => {
			expect(
				(await agent.getActivityState(makeSession({ runtimeHandle: null })))
					?.state
			).toBe("exited");
		});
		...
		it("returns null when no workspacePath", async () => {
			expect(
				await agent.getActivityState(makeSession({ workspacePath: null }))
			).toBeNull();
		});
		...
		describe("staleness threshold", () => {
			...
			it("respects custom readyThresholdMs", async () => {
				...
				expect(
					(await agent.getActivityState(makeSession(), 60_000))?.state
				).toBe("idle");
				expect(
					(await agent.getActivityState(makeSession(), 300_000))?.state
				).toBe("ready");
			});
		...
		describe("realistic session sequences", () => {
			...
			it("detects agent done and waiting (assistant is last entry)", async () => {
				writeJsonl([
					...
					{
						type: "assistant",
						message: { content: "I've implemented the auth feature." },
					},
				]);
				expect((await agent.getActivityState(makeSession()))?.state).toBe(
					"ready"
