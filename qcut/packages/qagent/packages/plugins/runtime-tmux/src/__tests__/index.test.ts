...
describe("runtime.create()", () => {
	...
	it("includes -e KEY=VALUE flags for environment variables", async () => {
		...
		await runtime.create({
			...
			launchCommand: "bash",
			environment: { QAGENT_SESSION: "env-session", FOO: "bar" },
		});
		...
		expect(args).toContain("QAGENT_SESSION=env-session");
	...
	it("cleans up session if send-keys fails", async () => {
		...
		await expect(
			...
			'Failed to send launch command to session "fail-session"'
		);
...
describe("runtime.sendMessage()", () => {
	...
	it("cleans up buffer and temp file on paste failure", async () => {
		...
		await expect(runtime.sendMessage(handle, longText)).rejects.toThrow(
			"paste-buffer failed"
		);
