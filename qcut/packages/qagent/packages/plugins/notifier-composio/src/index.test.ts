...
describe("notifier-composio", () => {
	...
	describe("create — config parsing", () => {
		...
		it("throws on invalid defaultApp", () => {
			expect(() =>
				create({ composioApiKey: "k", defaultApp: "telegram" })
			).toThrow('Invalid defaultApp: "telegram"');
		});

		it("accepts slack as defaultApp", () => {
			expect(() =>
				create({ composioApiKey: "k", defaultApp: "slack" })
			).not.toThrow();
		});

		it("accepts discord as defaultApp", () => {
			expect(() =>
				create({ composioApiKey: "k", defaultApp: "discord" })
			).not.toThrow();
		});
		...
		it("throws when gmail is defaultApp without emailTo", () => {
			expect(() =>
				create({ composioApiKey: "k", defaultApp: "gmail" })
			).toThrow("emailTo is required");
		});
	...
	describe("notify", () => {
		...
		it("includes prUrl when present as string", async () => {
			...
			await notifier.notify(
				makeEvent({ data: { prUrl: "https://github.com/pull/1" } })
			);
