...
export const manifest = {
	...
	description:
		"Notifier plugin: Composio unified notifications (Slack, Discord, email)",
	version: "0.1.0",
};
...
async function loadComposioSDK(
	...
	try {
		...
		const mod = (await import("composio-core")) as unknown as Record<
			string,
			unknown
		>;
...
function formatNotifyText(event: OrchestratorEvent): string {
	...
	const prUrl =
		typeof event.data.prUrl === "string" ? event.data.prUrl : undefined;
	if (prUrl) {
		parts.push(`PR: ${prUrl}`);
	}
...
export function create(config?: Record<string, unknown>): Notifier {
	const apiKey =
		(typeof config?.composioApiKey === "string"
			? config.composioApiKey
			: undefined) ?? process.env.COMPOSIO_API_KEY;
	const defaultApp: ComposioApp =
		typeof config?.defaultApp === "string" && VALID_APPS.has(config.defaultApp)
			...
			: "slack";
	const channelName =
		typeof config?.channelName === "string" ? config.channelName : undefined;
	const channelId =
		typeof config?.channelId === "string" ? config.channelId : undefined;
	const emailTo =
		typeof config?.emailTo === "string" ? config.emailTo : undefined;
	...
	const clientOverride =
		...
		typeof (config._clientOverride as ComposioToolkit).executeAction ===
			"function"
			? (config._clientOverride as ComposioToolkit)
			: undefined;

	if (
		typeof config?.defaultApp === "string" &&
		!VALID_APPS.has(config.defaultApp)
	) {
		throw new Error(
