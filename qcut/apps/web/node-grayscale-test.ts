...
function testGrayscaleConversionLogic(): void {
    ...
    testPixels.forEach(pixel => {
        ...
        console.log(`   ${pixel.name.padEnd(8)} (${r.toString().padStart(3)}, ${g.toString().padStart(3)}, ${b.toString().padStart(3)}) → ${luminance.toString().padStart(3)}`);
    });
    ...
    testPixels.forEach(pixel => {
        ...
        console.log(`   ${pixel.name.padEnd(8)} (${r.toString().padStart(3)}, ${g.toString().padStart(3)}, ${b.toString().padStart(3)}) → ${average.toString().padStart(3)}`);
    });
    ...
    testPixels.forEach(pixel => {
        ...
        console.log(`   ${pixel.name.padEnd(8)} (${r.toString().padStart(3)}, ${g.toString().padStart(3)}, ${b.toString().padStart(3)}) → ${grayValue.toString().padStart(3)}`);
    });
    ...
    console.log('');
    console.log('💡 The luminance formula gives the most visually accurate grayscale');
    console.log('   because it accounts for human eye sensitivity to different colors.');
...
function testImageDataConversion(): void {
    ...
    const redPixel: [number, number, number] = [imageData.data[0], imageData.data[1], imageData.data[2]];
    const bluePixel: [number, number, number] = [imageData.data[200], imageData.data[201], imageData.data[202]];
    ...
    const convertedRed: [number, number, number] = [grayscaleData.data[0], grayscaleData.data[1], grayscaleData.data[2]];
    const convertedBlue: [number, number, number] = [grayscaleData.data[200], grayscaleData.data[201], grayscaleData.data[202]];
    ...
    console.log(`  Red conversion: ${convertedRed[0]} (expected: ${expectedRed}) ${convertedRed[0] === expectedRed ? '✓' : '✗'}`);
    console.log(`  Blue conversion: ${convertedBlue[0]} (expected: ${expectedBlue}) ${convertedBlue[0] === expectedBlue ? '✓' : '✗'}`);
}
...
function main(): void {
    ...
    console.log('   This sets saturation to 0, which is what QCut uses for grayscale');
    console.log('');
    console.log('📝 Summary:');
