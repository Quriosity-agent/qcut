...
vi.mock('@/stores/media-store-loader', () => ({
  getMediaStoreUtils: vi.fn(() => Promise.resolve({
...
describe('useAspectRatio - Advanced Features', () => {
  ...
  it('formats aspect ratio with proper precision', () => {
    ...
    expect(result.current.formatAspectRatio(1.333333)).toBe('4:3'); // Close to 4:3
    expect(result.current.formatAspectRatio(1.7777)).toBe('16:9'); // Close to 16:9
    expect(result.current.formatAspectRatio(1.234567)).toBe('1.23'); // Custom ratio
  });
  ...
  it('handles error state properly', async () => {
    ...
    const { useAsyncMediaItems } = await import('@/hooks/use-async-media-store');
    const mockUseAsyncMediaItems = useAsyncMediaItems as ReturnType<typeof vi.fn>;
