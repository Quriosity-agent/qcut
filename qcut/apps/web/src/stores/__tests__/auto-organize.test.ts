...
describe("Auto-Organize Media", () => {
  ...
  describe("autoOrganizeByType", () => {
    it("should organize videos to Videos folder", async () => {
      ...
      expect(updatedItems.find((m) => m.id === "video-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.VIDEOS);
      expect(updatedItems.find((m) => m.id === "video-2")?.folderIds).toContain(DEFAULT_FOLDER_IDS.VIDEOS);
    });
    ...
    it("should organize audio to Audio folder", async () => {
      ...
      const updatedItems = useMediaStore.getState().mediaItems;
      expect(updatedItems.find((m) => m.id === "audio-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AUDIO);
      expect(updatedItems.find((m) => m.id === "audio-2")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AUDIO);
    });
    ...
    it("should organize images to Images folder", async () => {
      ...
      const updatedItems = useMediaStore.getState().mediaItems;
      expect(updatedItems.find((m) => m.id === "image-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.IMAGES);
      expect(updatedItems.find((m) => m.id === "image-2")?.folderIds).toContain(DEFAULT_FOLDER_IDS.IMAGES);
    });

    it("should organize AI-generated media to AI Generated folder", async () => {
      const mediaItems = [
        createMockMediaItem("ai-image-1", "image", { metadata: { source: "text2image" } }),
        createMockMediaItem("ai-video-1", "video", { metadata: { source: "fal-ai" } }),
      ];
      useMediaStore.setState({ mediaItems });
      ...
      expect(updatedItems.find((m) => m.id === "ai-image-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AI_GENERATED);
      expect(updatedItems.find((m) => m.id === "ai-video-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AI_GENERATED);
    });

    it("should skip already-organized media", async () => {
      const existingFolder = "custom-folder-123";
      const mediaItems = [
        createMockMediaItem("organized-1", "video", { folderIds: [existingFolder] }),
      ...
      expect(updatedItems.find((m) => m.id === "organized-1")?.folderIds).toEqual([existingFolder]);
      // Unorganized item should be organized
      expect(updatedItems.find((m) => m.id === "unorganized-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.VIDEOS);
    });
    ...
    it("should skip ephemeral media items", async () => {
      ...
      expect(updatedItems.find((m) => m.id === "ephemeral-1")?.folderIds || []).toHaveLength(0);
      // Regular items should be organized
      expect(updatedItems.find((m) => m.id === "regular-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.VIDEOS);
    });
    ...
    it("should handle mixed media types correctly", async () => {
      const mediaItems = [
        ...
        createMockMediaItem("audio-1", "audio"),
        createMockMediaItem("image-1", "image"),
        createMockMediaItem("ai-1", "image", { metadata: { source: "ai-gen" } }),
      ];
      useMediaStore.setState({ mediaItems });
      ...
      expect(updatedItems.find((m) => m.id === "audio-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AUDIO);
      expect(updatedItems.find((m) => m.id === "image-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.IMAGES);
      expect(updatedItems.find((m) => m.id === "ai-1")?.folderIds).toContain(DEFAULT_FOLDER_IDS.AI_GENERATED);
    });
    ...
    it("should not organize items that already have folders", async () => {
      const mediaItems = [
        createMockMediaItem("already-organized", "video", { folderIds: ["existing-folder"] }),
      ];
      useMediaStore.setState({ mediaItems });
  ...
  describe("getMediaByFolder", () => {
    ...
    it("should support media in multiple folders", () => {
      ...
      const mediaItems = [
        createMockMediaItem("multi-folder", "video", { folderIds: [folder1, folder2] }),
    ...
    it("should exclude ephemeral items from folder queries", () => {
      ...
      const mediaItems = [
        ...
        createMockMediaItem("ephemeral", "audio", { folderIds: [folderId], ephemeral: true }),
      ];
      useMediaStore.setState({ mediaItems });
