 ...
 *
 * Media generation actions → moyin-store-media.ts
 * Pipeline/calibration actions → moyin-store-pipeline.ts
 */
...
import { createMediaActions } from "./moyin-store-media";
import { createPipelineActions } from "./moyin-store-pipeline";
...
export const useMoyinStore = create<MoyinStore>((set, get) => {
	...
	const mediaActions = createMediaActions(set as any, get as any, patchShot);
	const pipelineActions = createPipelineActions(set as any, get as any);
	...
	return {
		...
		// Pipeline actions (parseScript, generateScript, enhance*, analyze*, storyboard)
		...pipelineActions,
		...
		// Media generation actions (generateShotImage, generateShotVideo, generateEndFrameImage)
		...mediaActions,
		...
		reorderShots: (shotId, targetIndex) =>
			set({
				shots: reorderShotsAction(shotId, targetIndex, get().shots),
			}),
