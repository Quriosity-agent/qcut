...
describe('Sounds API Migration', () => {
  ...
  describe('Feature Flag Testing', () => {
    ...
    it('should fall back to Next.js API when Electron API fails', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/sounds/search?q=thunder&type=effects');
    ...
    it('should use Next.js API when USE_ELECTRON_API is false', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('/api/sounds/search?q=music&type=effects&page=2&page_size=10');
      expect(result).toEqual(mockApiResponse);
    });
  });
  ...
  describe('Parameter Handling', () => {
    it('should handle all search parameters correctly', async () => {
      ...
      mockElectronAPI.sounds.search.mockResolvedValue({ success: true, results: [] });
