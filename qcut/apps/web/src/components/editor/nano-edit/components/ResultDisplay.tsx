...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ content, onUseImageAsInput, onImageClick, originalImageUrl }) => {
  ...
  const handleDownloadComparison = useCallback(async () => {
    ...
    const loadPromises = validImages.map(item => new Promise<boolean>((resolve) => {
        ...
        item.img.onerror = (error) => {
            console.warn(`Failed to load image for comparison: ${item.url}`, error);
            resolve(false);
        };
    ...
    try {
        ...
        if (loadedImages.length < 2) {
            console.warn('Insufficient images loaded for comparison, falling back to individual downloads');
            handleDownloadBoth();
            return;
        }
        ...
        {twoStepViewMode === 'slider' && (
                ...
                >
                    ...
                    <div className="absolute top-0 bottom-0 bg-orange-500 w-1 cursor-ew-resize" style={{ left: `calc(${sliderPosition}% - 2px)` }}>
                        <div className="absolute top-1/2 -translate-y-1/2 -left-3.5 bg-orange-500 h-8 w-8 rounded-full border-2 border-black flex items-center justify-center text-black">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" d="M8 9l4-4 4 4m0 6l-4 4-4-4" /></svg>
                        </div>
                    </div>
                </div>
            </div>
         ...
         {twoStepViewMode !== 'result' && (
            <div className="w-full flex flex-col md:flex-row gap-3 mt-auto">
                ...
                >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path d="M5 4a1 1 0 00-2 0v7.268a2 2 0 000 3.464V16a1 1 0 102 0v-1.268a2 2 0 000-3.464V4zM11 4a1 1 0 10-2 0v1.268a2 2 0 000 3.464V16a1 1 0 102 0V8.732a2 2 0 000-3.464V4zM15 3a1 1 0 011 1v12a1 1 0 11-2 0V4a1 1 0 011-1z" /></svg>
        ...
        {viewMode === 'result' && content.imageUrl && (
          ...
          >
            ...
            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
               <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" aria-hidden="true">
