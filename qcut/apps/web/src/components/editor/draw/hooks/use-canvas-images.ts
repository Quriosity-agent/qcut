...
export const useCanvasImages = (
  ...
  const updateImage = useCallback(
    (id: string, updates: Partial<ImageTransform>) => {
      setImages((prev) =>
  ...
  const getImageAtPosition = useCallback(
    (x: number, y: number): CanvasImage | null => {
      ...
      for (let i = images.length - 1; i >= 0; i--) {
        ...
        if (
          rotatedX >= -img.width / 2 &&
          rotatedX <= img.width / 2 &&
          rotatedY >= -img.height / 2 &&
          rotatedY <= img.height / 2
        ) {
          return img;
        }
  ...
  const createGroup = useCallback(
    (name?: string) => {
      ...
      setImages((prev) =>
        prev.map((img) =>
          selectedImageIds.includes(img.id) ? { ...img, groupId } : img
        )
      );
  ...
  const startDrag = useCallback(
    (imageId: string, startX: number, startY: number) => {
    ...
    (currentX: number, currentY: number) => {
      if (isDragging && selectedImageIds.length > 0) {
        ...
        selectedImageIds.forEach((imageId) => {
          ...
          if (image) {
            const newX =
              image.x + (deltaX - (dragOffset.current.lastDeltaX || 0));
            const newY =
              image.y + (deltaY - (dragOffset.current.lastDeltaY || 0));
