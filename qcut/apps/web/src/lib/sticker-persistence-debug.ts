...
export async function debugStickerPersistence() {
  ...
  console.log(`🎯 Current stickers (${currentStickers.length}):`, currentStickers);
  
  // Check media items
  const mediaItems = mediaStore.mediaItems;
  console.log(`📹 Media items (${mediaItems.length}):`, mediaItems.map(m => ({
    id: m.id,
    name: m.name,
    type: m.type,
    hasFile: !!m.file,
  ...
  for (const sticker of currentStickers) {
    ...
    if (media) {
      console.log(`✅ Sticker ${sticker.id} -> Media ${media.name} (${media.id})`);
    } else {
      console.error(`❌ Sticker ${sticker.id} -> Media NOT FOUND (${sticker.mediaItemId})`);
    }
  }
  ...
  if (savedStickers.length > 0 && currentStickers.length > 0) {
    ...
    if (mismatch) {
      console.error("⚠️ ID mismatch detected between saved and current stickers!");
    }
