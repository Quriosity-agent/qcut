 ...
 */
export const setAudioExportConfig = (config: Partial<AudioExportConfig>): void => {
 ...
 */
export const getCodecForFormat = (format: string): 'aac' | 'opus' | 'mp3' => {
  const formatCodecMap: Record<string, 'aac' | 'opus' | 'mp3'> = {
  ...
  return formatCodecMap[format.toLowerCase()] || formatCodecMap.default;
};
 ...
 */
export const validateAudioConfig = (config: Partial<AudioExportConfig>): boolean => {
  if (config.bitrate && (config.bitrate < 32 || config.bitrate > 320)) {
    console.warn(`Invalid audio bitrate: ${config.bitrate}. Must be between 32 and 320 kbps.`);
    return false;
  }
  
  if (config.sampleRate && ![22050, 44100, 48000, 96000].includes(config.sampleRate)) {
    console.warn(`Unusual sample rate: ${config.sampleRate}. Common rates are 22050, 44100, 48000, 96000 Hz.`);
  }
