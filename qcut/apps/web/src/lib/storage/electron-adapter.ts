...
export class ElectronStorageAdapter<T> implements StorageAdapter<T> {
  ...
  async get(key: string): Promise<T | null> {
    try {
      ...
      if (!api?.storage?.load) {
        handleStorageError(new Error("Electron API unavailable"), "Get data from Electron storage", { key, operation: "get" });
        return null;
      }
  ...
  async set(key: string, value: T): Promise<void> {
    try {
      ...
      const api = this.getElectronAPI();
      if (!api?.storage?.save) {
        handleStorageError(new Error("Electron API unavailable"), "Save data to Electron storage", { key, operation: "set" });
  ...
  async remove(key: string): Promise<void> {
    try {
      ...
      if (!api?.storage?.remove) {
        handleStorageError(new Error("Electron API unavailable"), "Remove data from Electron storage", { key, operation: "remove" });
        throw new Error("Electron API unavailable");
      }
  ...
  async list(): Promise<string[]> {
    try {
      const api = this.getElectronAPI();
      if (!api?.storage?.list) {
        handleStorageError(new Error("Electron API unavailable"), "List Electron storage keys", { operation: "list" });
        throw new Error("Electron API unavailable");
      }
      const allKeys = await api.storage.list();
      if (!Array.isArray(allKeys)) {
        handleStorageError(new Error("Electron storage.list returned non-array"), "List Electron storage keys", { operation: "list" });
  ...
  async clear(): Promise<void> {
    try {
      ...
      if (!api?.storage?.list || !api?.storage?.remove) {
        handleStorageError(new Error("Electron API unavailable"), "Clear Electron storage", { operation: "clear" });
        throw new Error("Electron API unavailable");
      }
      const allKeys = await api.storage.list();
